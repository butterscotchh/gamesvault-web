...
const ProductCarousel = () => {
  ...
  const NavBtn = ({ onClick, label, children }) => (
    <button
      ...
      className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center transition-all hover:scale-110"
      style={{ 
        background: '#dedad0', 
      ...
      onMouseEnter={e => { 
        ...
        e.currentTarget.style.background = '#d5d0c0';
      }}
      onMouseLeave={e => { 
        e.currentTarget.style.borderColor = '#bfbaa7'; 
  ...
  return (
    <div className="w-full px-1 md:px-6 overflow-visible">
      <div className="relative overflow-visible">
        ...
        {totalPages > 1 && (
          <>
            <div className="absolute -left-3 md:-left-20 top-1/2 -translate-y-1/2 z-10">
              <NavBtn onClick={prevPage} label="Previous">
                <ChevronLeft className="w-4 h-4 md:w-5 md:h-5" />
              </NavBtn>
            </div>
            <div className="absolute -right-3 md:-right-20 top-1/2 -translate-y-1/2 z-10">
              <NavBtn onClick={nextPage} label="Next">
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-3 md:gap-5 mt-6 md:mt-10">
          <span style={{ 
          ...
          <div style={{ width: 4, height: 4, background: '#8a7a60', transform: 'rotate(45deg)' }} />
        </div>
      )}
    </div>
  );
